...
public class Limb {
	 ...
	 * @param range
	 *            The maximum range (in encoder ticks) that the motor is
	 *            supposed to make
	 */
	public Limb(final RegulatedMotor motor, final int range) {
		this.motor = motor;
		this.range = range;
	}
	...
	/**
	 * Calibrates the position of the motor by slowly moving it to an end point.
	 * Also sets the tacho counter to zero after calibrating.
	 * 
	 * @param stallDetectionSpeed
	 *            The speed of the motor (in degrees/second) that must by used
	 *            while calibrating
	 * @param stallDirection
	 *            The direction the motor must rotate to while calibrating
	 *            (1=forward, -1=backward)
	 * @param stallToRest
	 *            The number of ticks the motor must rotate back from the
	 *            detected stall position. It is assumed that the stall position
	 *            over stretches the robot. Rotating a bit back from the stall
	 *            position releases tension of the robot. The tacho is reset
	 *            after rotating back from the stall position
	 * @param speedFactor
	 *            Sets the speed after calibrationg to this value. Expressed a a
	 *            fraction of the maximum speed.
	 */
	public void calibrate(final int stallDetectionSpeed,
			final int stallDirection, final int stallToRest,
	...
	/**
	 * Sets a logical range for the limb. The logical range is used to hide the
	 * tacho range of the motor.
	 * 
	 * @param min
	 ...
	 */
	public void setLogicRange(final int min, final int max) {
		this.min = min;
		this.max = max;
	...
	public void moveTo(final float value, final boolean immediateReturn) {
		if (tickFactor != 0) {
			target = (int) value;
			motor.rotateTo(toTick(target), immediateReturn);
		}
	}
